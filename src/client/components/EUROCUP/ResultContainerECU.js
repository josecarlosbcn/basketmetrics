...
import { RESULT_GAMES_FIBA } from "../FIBACOM/Querys";
...
class ResultContainerECU extends Component{
    constructor(props){
        super();
        this.props = props;
        /**
        ...
        console.log("Load of Component ResultContainerECU");     
    }
    ...
    async getData(){
        ...
        let games = await getDataFromServer(process.env.URL_FIBA_API, RESULT_GAMES_FIBA, params);
        //console.log("Total partidos: " + games["data"]["lf1_games"].length);
        this.setState({
            games: games.data.fiba_games
        });
    }
    ...
    render(){
        let games = this.state.games.map( game => {
            let url_std = "game/standard-stats/" + game.home_url_name + "--" + game.away_url_name + "/" + game.id;
            let url_adv = "game/advanced-stats/" + game.home_url_name + "--" + game.away_url_name + "/" + game.id;            
            return {
                id_game: game.id,
...
module.exports.ResultContainerECU = ResultContainerECU;