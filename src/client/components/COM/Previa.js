...
import {PREVIA_NEXT_GAME_LF1, PREVIA_NEXT_GAME_LF2, PREVIA_TEAM, PREVIA_TCSS, TEAM_PREVIA_ADV_STATS, 
        PREVIA_PLAYER, TEAM_PREVIA_STD_STATS} from "../FEBCOM/Querys";
 ...
 */
/**
 * history object is inherited due to this module is called from SubMenu module who is exported with withRouter(SubMenu)
 */
export default class Previa extends Component{
    constructor(){
        super();
        let expr_lf1 = /^(http|https):\/\/[A-Za-z0-9-.]*[:|[0-9]*]?\/liga-femenina-1\/\w*\/[A-Z-]*\/\d*$/;
        let expr_lf2 = /^(http|https):\/\/[A-Za-z0-9-.]*[:|[0-9]*]?\/liga-femenina-2\/\w*\/[A-Z-]*\/\d*$/;        
        let isLF1 = expr_lf1.test(window.location.href);
        let isLF2 = expr_lf2.test(window.location.href);
        console.log("lf1: " + isLF1 + " lf2: " + isLF2);
        ...
        this.state = {
            ...
            local_previa_player: {},
            away_previa_player: {},
            lang: (isLF1 || isLF2) ? "es" : "en"
        };
    }

    getQuery(){
        if (this.state.isLF1)
            return PREVIA_NEXT_GAME_LF1;
        if (this.state.isLF2)
            return PREVIA_NEXT_GAME_LF2;
    }
    ...
    async getData(){
        ...
        let game = await getDataFromServer(process.env.URL_FEB_API, this.getQuery(), params);

        if (this.state.isLF1)
            params = [{"###param1###" : game.data.list_nextgamedaylf1[0].id_home_team}];
        if (this.state.isLF2)
            params = [{"###param1###" : game.data.list_nextgamedaylf2[0].id_home_team}];
        let local = await getDataFromServer(process.env.URL_FEB_API, PREVIA_TEAM, params);

        if (this.state.isLF1)
            params = [{"###param1###" : game.data.list_nextgamedaylf1[0].id_away_team}];
        if (this.state.isLF2)
            params = [{"###param1###" : game.data.list_nextgamedaylf2[0].id_away_team}];
        let away = await getDataFromServer(this.getURLApi(), PREVIA_TEAM, params);

        let params_local = [{"###id_team_club###" : local.data.teamclub[0].id}];
        let params_away = [{"###id_team_club###" : away.data.teamclub[0].id}];

        let local_tcss = await getDataFromServer(process.env.URL_FEB_API, PREVIA_TCSS, params_local);
        let local_team_std_stats = await getDataFromServer(process.env.URL_FEB_API, TEAM_PREVIA_STD_STATS, params_local);
        let away_tcss = await getDataFromServer(process.env.URL_FEB_API, PREVIA_TCSS, params_away);
        let away_team_std_stats = await getDataFromServer(process.env.URL_FEB_API, TEAM_PREVIA_STD_STATS, params_away);
        let local_tcas = await getDataFromServer(process.env.URL_FEB_API, TEAM_PREVIA_ADV_STATS, params_local);
        let away_tcas = await getDataFromServer(process.env.URL_FEB_API, TEAM_PREVIA_ADV_STATS, params_away);
        let local_previa_player = await getDataFromServer(process.env.URL_FEB_API, PREVIA_PLAYER, params_local);
        let away_previa_player = await getDataFromServer(process.env.URL_FEB_API, PREVIA_PLAYER, params_away);
        ...
        g3_data = [
            ...
            {
                name: (this.state.lang === "es") ? "% T3P Points" : "% 3P Points",
            ...
            <div style = {{marginTop: 15 + 'px'}}>                          
                ...
                <Row>
                    ...
                    <Col md = {2} style = {{textAlign: "right", marginTop: 15 + "px"}}>
                        <Tooltip content = {(this.state.lang === "es") ? "Volver a la página anterior" : "Go to previous page"}>
                            <Button
                ...
                <Row>
                    ...
                    <Col md = {2} style = {{textAlign: "right", marginTop: 15 + "px"}}>
                        <Tooltip content = {(this.state.lang === "es") ? "Volver a la página anterior" : "Back previous page"}>
                            <Button
                                onClick = {() => history.back()}
                                style = {{width: 100 + "%"}}
                            >
