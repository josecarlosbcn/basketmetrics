...
class ScatterPlot3 extends Component {
  /**
   * We are going to use this development for values between -1 and 1 with little deviations. (@Improve!!!)
  ...
  componentDidUpdate(prevProps){
    ...
    if ((typeof this.props.visible === "undefined" || this.props.visible) && (typeof this.props.data !== "undefined")){
        console.log("ScatterPlot2: VISIBLE: Mostramos gráfico");
  ...
  drawGraphic(){
    ...
    this.setGrid(canvas, scales, this.props.percentage_x, this.props.percentage_y);
  ...
  setScales(data) {
    ...
    let xScale = d3
                .scaleLinear()
                .domain([
                  d3.min(data, d => parseFloat(d.value_x) - 0.1),
                  d3.max(data, d => parseFloat(d.value_x) + 0.1)
                ])
                .range(xRange);
    ...
    let yScale = d3
                .scaleLinear()
                .domain([
                  d3.max(data, d => parseFloat(d.value_y) + 0.1),
                  d3.min(data, d => parseFloat(d.value_y) - 0.1)
                ])
                .range(yRange);
  ...
  setAxesToCanvas(canvas, scales) {
    ...
    canvas
      ...
      .attr("y", -(this.state.OFFSET_LEFT / 2 + 20))
...
module.exports.ScatterPlot3 = ScatterPlot3;
