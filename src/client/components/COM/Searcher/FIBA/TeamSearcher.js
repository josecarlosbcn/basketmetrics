...
import { CLUB_SEASON_FIBA, TEAM_SEASON_STD_STATS_FIBA, TEAMS_SEASON_STD_STATS_FIBA_2, TEAM_SEASON_ADV_STATS_FIBA, TEAMS_SEASON_ADV_STATS_FIBA_2,
        TEAM_SEASON_MARGINS_FIBA, TEAMS_SEASON_MARGINS_FIBA, TEAM_SEASON_WINS_FIBA, TEAMS_SEASON_WINS_FIBA, TEAM_OPP_STDSTATS_BY_SEASON_FIBA, 
        TEAM_OPP_ADVSTATS_BY_SEASON_FIBA, TEAM_GAME_LOG_FIBA, TEAM_STDSTATS_HISTORY_FIBA, TEAM_ADVSTATS_HISTORY_FIBA } from "../../../FIBACOM/Querys";
...
class TeamSearcher extends Component {
    ...
    async getData(){
        ...
        let seasons = await getDataFromServer(process.env.URL_FIBA_API, CLUB_SEASON_FIBA, params);
        let params2 = [{"###id_club###" : this.props.item.id}, {"###id_season###" : seasons.data.club_season[0].id}];
        let team_std_stats = await getDataFromServer(process.env.URL_FIBA_API, TEAM_SEASON_STD_STATS_FIBA, params2);
        let params1 = [{"###id_season###" : seasons.data.club_season[0].id}];
        let teams_std_stats = await getDataFromServer(process.env.URL_FIBA_API, TEAMS_SEASON_STD_STATS_FIBA_2, params1);
        let team_adv_stats = await getDataFromServer(process.env.URL_FIBA_API, TEAM_SEASON_ADV_STATS_FIBA, params2);
        let teams_adv_stats = await getDataFromServer(process.env.URL_FIBA_API, TEAMS_SEASON_ADV_STATS_FIBA_2, params1);
        let team_margins = await getDataFromServer(process.env.URL_FIBA_API, TEAM_SEASON_MARGINS_FIBA, params2);
        let teams_margins = await getDataFromServer(process.env.URL_FIBA_API, TEAMS_SEASON_MARGINS_FIBA, params1);
        let team_wins = await getDataFromServer(process.env.URL_FIBA_API, TEAM_SEASON_WINS_FIBA, params2);
        let teams_wins = await getDataFromServer(process.env.URL_FIBA_API, TEAMS_SEASON_WINS_FIBA, params1);
        let team_opp_std_stats = await getDataFromServer(process.env.URL_FIBA_API, TEAM_OPP_STDSTATS_BY_SEASON_FIBA, params2);
        let team_opp_adv_stats = await getDataFromServer(process.env.URL_FIBA_API, TEAM_OPP_ADVSTATS_BY_SEASON_FIBA, params2);
        let team_game_log = await getDataFromServer(process.env.URL_FIBA_API, TEAM_GAME_LOG_FIBA, params2);
        let team_stdstats_history = await getDataFromServer(process.env.URL_FIBA_API, TEAM_STDSTATS_HISTORY_FIBA, params);
        ...
        let team_advstats_history = await getDataFromServer(process.env.URL_FIBA_API, TEAM_ADVSTATS_HISTORY_FIBA, params);
                        ...
                        <div>
                            ...
                            <div style = {{display: (this.state.statSelected === 0) ? "block" : "none"}}>
                                <div className = "font-weight-bold text-center"
                                    ...
                                    {this.state.seasonDescription} :: {this.state.teamName}
                                </div>
                                <Row>
                                    <Col md = {6}>
