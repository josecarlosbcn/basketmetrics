import { GraphQLObjectType, GraphQLSchema, GraphQLList, GraphQLID, GraphQLInt, GraphQLString } from "graphql";
import { FIBAConnection }  from "../db";
...
import { PlayerCompetitions } from "./player.competitions.schema.js";
//import { transformStringToArray } from "../../client/components/COM/Functions";
...
const QueryFIBA = new GraphQLObjectType({
    name: "QueryFIBA",
    ...
    fields: () => {
        return {
            boxscore: {
                ...
                resolve(root, args){
                    let query = "select bx.id_game, bx.id_pc as id_player_team, tc.name as team_name, pt.numero, p.name, bx.titular, bx.minutes, bx.seconds, " + 
                    ...
                    "tbl003_player p where bx.id_game = ###id_game### and tc.id = ###id_team_club### and bx.id_pc = pt.id and " +
                    ...
                    return FIBAConnection.db.query(
                        query,
                        {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
                    );
                }
            },            
            club_season: {
                ...
                resolve(root, args){
                    let query = "select s.id, l.name as leagueName, concat(l.name, ' / ', s.description) as description, tc.name from tbl002_club c, " +
                    "tbl005_team_club tc, tbl015_seasons s, tbl001_league l where c.id = ###id_club### and tc.id_club = c.id and s.id = tc.id_season " +
                    "and s.id_league = l.id order by s.id desc";
            ...
            eurocup_nextgameday: {
                ...
                description: "Lista de partidos de la próxima jornada para Eurocup",
                args: {
                    id: {
                        type: GraphQLInt
                    }
                ...
                resolve(root, args){
                    if (typeof args.id === "undefined"){
                        return FIBAConnection.db.query(
                            "select ng.*, tc1.name as home_team, tc1.url_name as home_url_name, tc2.name as away_team, tc2.url_name as away_url_name, c.name as competition from tbl032_next_game_eurocup ng, tbl005_team_club tc1, tbl005_team_club tc2, tbl012_competition c where ng.played = 0 and ng.id_home_team = tc1.id and ng.id_away_team = tc2.id and ng.id_competition = c.id",
                            {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
                        );
                    }
                    else{
                        return FIBAConnection.db.models.tbl032_next_game_eurocup.findAll({where: args});
                    }
                }                
            },
            euroleague_nextgameday: {
                type: GraphQLList(NextGameday),
                description: "Lista de partidos de la próxima jornada para Euroleague",
                args: {
                    id: {
                        type: GraphQLInt
                    }
                ...
                resolve(root, args){
                    if (typeof args.id === "undefined"){
                        return FIBAConnection.db.query(
                            "select ng.*, tc1.name as home_team, tc1.url_name as home_url_name, tc2.name as away_team, tc2.url_name as away_url_name, c.name as competition from tbl031_next_game_euroleague ng, tbl005_team_club tc1, tbl005_team_club tc2, tbl012_competition c where ng.played = 0 and ng.id_home_team = tc1.id and ng.id_away_team = tc2.id and ng.id_competition = c.id",
                            {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
                        );
                    }
                    else{
                        return FIBAConnection.db.models.tbl031_next_game_euroleague.findAll({where: args});
                    }
                }                
            },
            fiba_games: {
                type: GraphQLList(Game),
                description: "List of games of a gameday for Euroleague and Eurocup Women",
                args: {
                    id_jornada: {
                        type: GraphQLInt
                    }
            ...
            game_adv_stats:{
                ...
                resolve(root, args){
                    return FIBAConnection.db.models.tbl009_team_adv_stats.findAll({where: args});
                }
            },            
            game_data :{
                type: GraphQLList(Game),
            ...
            game_log: {
                ...
                resolve(root, args){
                    let query = "select g.id as id_game, g.date_game, c.id, tc1.url_name as home_team_url, tc1.abrev as home_abrev, tc2.url_name as away_team_url, " +
                    ...
                    "bx.assists, bx.turnovers, bx.steals, bx.block_shots, bx.fouls_cm, bx.efficience as efficience " +
                    "from tbl003_player p, tbl006_player_team pt, tbl011_boxscore bx, tbl007_game g, tbl004_jornada j, tbl012_competition c, " + 
                    "tbl005_team_club tc1, tbl005_team_club tc2 where p.id = ###id_player### and p.id = pt.id_player and pt.id = bx.id_pc and " +
                    "bx.id_game = g.id and g.id_team_home = tc1.id and g.id_team_away = tc2.id and g.id_jornada = j.id and j.id_competition = c.id and " +
                    "c.id in (###id_comps###) order by g.date_game asc";
                    let comps = transformStringToArray(args.id_comps);
                    query = query.replace("###id_player###", args.id_player);
                    query = query.replace("###id_comps###", comps);
                    return FIBAConnection.db.query(
                        query,
                        {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
                    );
            ...
            last_game_day: {
                ...
                resolve(root, args){
                    ...
                    // "j.num_jornada, j.date, j.date_insert from tbl004_jornada j, tbl012_competition c, tbl015_seasons s, " +
                    ...
                    // "c.id_season = s.id";
                    let query = "select s.id as id_season, s.description as season, j.id_competition, c.name as competition, j.id as id_jornada, j.num_jornada, j.date, j.date_insert " +
                    "from tbl004_jornada j, tbl012_competition c, tbl015_seasons s, (select j.id from tbl004_jornada j, " +
                    "(select id from tbl012_competition where id_league = ###id_league### order by id desc limit 1) c where j.id_competition = c.id " +
                    "order by id desc limit 1) lj where j.id = lj.id and j.id_competition = c.id and c.id_season = s.id";
                    query = query.replace("###id_league###", args.id_league);
                    return FIBAConnection.db.query(
                        query,
                        {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
            ...
            league_stats: {
                ...
                resolve(root, args){
                    ...
                    "pt.id_team_club = tc2.id and pt.id = bx.id_pc) t1, (select round(avg(g.points), 2) as pbg, " + 
                    ...
                    "pt.id_team_club = tc2.id and pt.id = pas.id_player) t3";
            ...
            list_competitions: {
                type: GraphQLList(Competition),
                description: "List of competitions given a league and a season",
                args: {
                    id_league: {
                        type: GraphQLInt
                    },
                ...
                resolve(root, args){
                    let query = "select * from tbl012_competition where id_league = ###id_league### and id_season = ###id_season###";
                    query = query.replace(/###id_league###/g, args.id_league);
                    query = query.replace(/###id_season###/g, args.id_season);
                    return FIBAConnection.db.query(
                        query,
                        {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
            ...
            list_game_days: {
                ...
                resolve(root, args){
                    let query = "select id as id_jornada, id_competition, num_jornada, date, date_insert from " +
                    "tbl004_jornada where id_competition = ###id_competition###";
                    query = query.replace("###id_competition###", args.id_competition);
                    return FIBAConnection.db.query(
                        query,
                        {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
            ...
            player_advanced_stats: {
                ...
                resolve(root, args){
                    let query = "select tc.id as id_team_club, tc.name as team_name, pt.numero, p.name as name_player, pas.id_player as id_player_team, " + 
                    "pas.id_game, pas.id_competition, pas.dre, pas.game_score, pas.per, pas.etc, pas.ts, pas.usg, pas.p_reb_def, pas.p_reb_of, " +
                    "pas.p_tot_reb, pas.p_assists, pas.assists_x_turnovers, pas.assists_ratio, pas.p_steals, pas.ortg, pas.drtg, pas.nrtg, pas.date_insert " + 
                    "from tbl013_player_adv_stats pas, " +
                    "tbl005_team_club tc, tbl006_player_team pt, tbl003_player p where pas.id_game = ###id_game### and pas.id_player = pt.id and " +
            ...
            player_adv_stats: {
                type: GraphQLList(PlayerAdvStats),
                description: "Advanced stats of a player in competitions",
                args: {
                    id_player: {
                        type: GraphQLInt
                    },
                ...
                resolve(root, args){
                    let query = "select p.id as id_player, count(*) as games, round(sum(pas.dre)/count(*), 2) as dre, round(sum(pas.game_score)/count(*), 2) as game_score, " +
                    "round(sum(pas.per)/count(*), 2) as per, round(sum(pas.etc)/count(*), 2) as etc, round(sum(pas.ts)/count(*), 2) as ts, " +
                    "round(sum(pas.usg)/count(*), 2) as usg, round(sum(pas.p_reb_def)/count(*), 2) as p_reb_def, " +
                    "round(sum(pas.p_reb_of)/count(*), 2) as p_reb_of, round(sum(pas.p_tot_reb)/count(*), 2) as p_tot_reb, " +
                    "round(sum(pas.p_assists)/count(*), 2) as p_assists, round(sum(pas.assists_x_turnovers)/count(*), 2) as assists_x_turnovers, " +
                    "round(sum(pas.assists_ratio)/count(*), 2) as assists_ratio, round(sum(pas.p_steals)/count(*), 2) as p_steals, " +
                    "round(sum(pas.ortg)/count(*), 2) as ortg, round(sum(pas.drtg)/count(*), 2) as drtg, round(sum(pas.nrtg)/count(*), 2) as nrtg " +
                    "from tbl003_player p, tbl006_player_team pt, tbl005_team_club tc, tbl013_player_adv_stats pas where " +
                    "p.id = ###id_player### and p.id = pt.id_player and pt.id_team_club = tc.id and pt.id = pas.id_player and " +
                    "pas.id_competition in (###id_comps###)";
                    let comps = transformStringToArray(args.id_comps);
                    query = query.replace("###id_player###", args.id_player);
                    query = query.replace("###id_comps###", comps);
                    return FIBAConnection.db.query(
            ...
            player_competitions: {
                type: GraphQLList(PlayerCompetitions),
...
/*                     let query = "select s.id, s.description, tc.name as team, l.name as league from tbl006_player_team pt, tbl005_team_club tc, tbl015_seasons s, " +
                    ...
                    "order by s.id desc"; */
                    let query = "select c.id, c.id_league, c.id_season, c.id_group, l.name, s.description from tbl003_player p, tbl006_player_team pt, " +
                    "tbl010_player_std_stats pss, tbl012_competition c, tbl001_league l, tbl015_seasons s where p.id = ###id_player### and " +
                    "p.id = pt.id_player and pt.id = pss.id_player and pss.id_competition = c.id and c.id_league = l.id and c.id_season = s.id order by c.id asc";
                    query = query.replace("###id_player###", args.id_player);
                    return FIBAConnection.db.query(
                        query,
                        {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
            ...
            player_std_stats: {
                ...
                description: "Standard stats of a player by team",
                args: {
                    id_player: {
                        type: GraphQLInt
                    },
                ...
                resolve(root, args){
                    ...
                    "sum(efficience) as efficience, round(sum(efficience)/sum(games), 2) as efficience_pp " +
                    "from tbl003_player p, tbl006_player_team pt, tbl010_player_std_stats pss where p.id = ###id_player### and p.id = pt.id_player and " +
                    "pss.id_player = pt.id and pss.id_competition in (###id_comps###);";
                    let comps = transformStringToArray(args.id_comps);
                    query = query.replace("###id_player###", args.id_player);
                    query = query.replace("###id_comps###", comps);
                    return FIBAConnection.db.query(
            ...
            players_adv_stats_2: {
                ...
                resolve(root, args){
                    ...
                    "pas.id_season = ###id_season### and pas.id_player_team = pss.id_player_team";
                    query = query.replace("###id_season###", args.id_season);
                    return FIBAConnection.db.query(
                        query,
                        {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
            ...
            player_stdstats_history: {
                ...
                resolve(root, args){
                    let query = "select s.description as season_name, pss.*, pas.assists_x_turnovers as astov, pss.reb_def as rd, " + 
            ...
            players_std_stats_2: {
                ...
                resolve(root, args){
                    let query = "select concat(p.name, ' (', tc.abrev, ') ') as name_player, pss.*, pas.assists_x_turnovers as astov, " + 
                    "pss.reb_def as rd, pss.reb_of as ro, pss.total_rebs as tot_reb, pss.turnovers as tov from tbl038_player_season_standard_stats pss, " + 
                    "tbl006_player_team pt, tbl003_player p, tbl005_team_club tc, tbl039_player_season_advanced_stats pas where pss.id_season = ###id_season### and " +
                    "pss.id_player_team = pt.id and pt.id_player = p.id and pt.id_team_club = tc.id and pss.id_player_team = pas.id_player_team";                    
                    query = query.replace("###id_season###", args.id_season);
            ...
            seasons: {
                ...
                resolve(root, args){
                    return FIBAConnection.db.models.tbl015_seasons.findAll({
                        where: args,
                        order: [
                            ["id", "DESC"]
                        ]
            ...
            teamclub: {
                ...
                resolve(root, args){
                    return FIBAConnection.db.models.tbl005_team_club.findAll({where: args});
                }
            },
            ...
            team_advstats_history: {
                ...
                resolve(root, args){
                    let query = "select concat(l.name, ' / ', s.description) as season_name, tc.id_club, tss.games, round(tss.total_puntos/tas.possessions, 2) as puntos_x_possession, " +
                    "round(tas.possessions/tss.games, 2) as pbg, tas.* from tbl005_team_club tc, tbl037_team_season_advanced_stats tas, tbl035_team_season_standard_stats tss, " + 
                    "tbl015_seasons s, tbl001_league l where tc.id_club = ###id_club### and tc.id = tas.id_team_club and s.id = tc.id_season and tss.id_team_club = tas.id_team_club " +
                    "and s.id_league = l.id order by s.description desc";                    
                    query = query.replace(/###id_club###/g, args.id_club);
                    return FIBAConnection.db.query(
                        query,
                        {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
            ...
            team_stdstats_history: {
                ...
                resolve(root, args){
                    let query = "select concat(l.name, ' / ', s.description) as season_name, tc.id_club, coalesce(round(tss.assists/tss.turnovers, 2), " + 
                    "tss.assists) as astov, tss.* from tbl005_team_club tc, tbl035_team_season_standard_stats tss, tbl015_seasons s, tbl001_league l where " +
                    "tc.id_club = ###id_club### and tc.id = tss.id_team_club and s.id = tc.id_season and s.id_league = l.id order by s.description desc";
                    query = query.replace("###id_club###", args.id_club);
                    return FIBAConnection.db.query(
                        query,
                        {raw: true, type: FIBAConnection.db.QueryTypes.SELECT}
            ...
            teams_by_season: {
                ...
                resolve(root, args){
                    let query = "select tc.* from tbl034_team_group tg, tbl033_groups g, tbl005_team_club tc where tg.id_group = g.id and " +
                    "g.id_league = ###id_league### and g.id_season = ###id_season### and tc.id = tg.id_team_club order by tc.name";
                    query = query.replace(/###id_season###/g, args.id_season);
                    query = query.replace(/###id_league###/g, args.id_league);
                    return FIBAConnection.db.query(
                        query,
            ...
            team_tcss: {
                ...
                resolve(root, args){
                    let query = "select t1.id_team_club, round(sum(t1.victories)/count(*), 0) as victories, sum(t1.games) as games, " + 
                    "sum(t1.t2p_conv) as t2p_conv, sum(t1.t2p_int) as t2p_int, sum(t1.t3p_conv) as t3p_conv, sum(t1.t3p_int) as t3p_int, " + 
                    "sum(t1.tl_conv) as tl_conv, sum(t1.tl_int) as tl_int, sum(t1.reb_def) as reb_def, sum(t1.reb_of) as reb_of, " +
                    "sum(t1.assists) as assists, sum(t1.steals) as steals, sum(t1.turnovers) as turnovers from " + 
...
const SchemaFIBA = new GraphQLSchema({
    query: QueryFIBA
});

module.exports.SchemaFIBA = SchemaFIBA;